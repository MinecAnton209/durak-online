import postgres from 'postgres';
import { drizzle } from 'drizzle-orm/postgres-js';
import { schema, tables } from './schema.js';
import 'dotenv/config';
import { eq, and, or, inArray, sql } from 'drizzle-orm';

import { user, profile, achievement, userAchievement, activeSession, donation, game, gameParticipant, friend, pushSubscription, inboxMessage, adminAuditLog, chatMessage, chatFilter, bannedDevice, knownDevice, systemStatsDaily, userDevice } from './schema.js';

type DrizzleDB = ReturnType<typeof drizzle>;

let _db: DrizzleDB | undefined;

export function initializeDb(connectionString = process.env.DATABASE_URL): DrizzleDB {
  if (!connectionString) {
    throw new Error('DATABASE_URL is not set before DB init');
  }
  if (!_db) {
    const client = postgres(connectionString, { max: 10 });
    _db = drizzle(client, { schema });
  }
  return _db;
}

export function getDb(): DrizzleDB {
  return initializeDb();
}

const TABLES: Record<string, any> = {
  user, profile, achievement, userAchievement, activeSession, donation, game,
  gameParticipant, friend, pushSubscription, inboxMessage, adminAuditLog,
  chatMessage, chatFilter, bannedDevice, knownDevice, systemStatsDaily, userDevice,
};

function buildCondition(table: Record<string, any>, key: string, value: any): any {
  if (key === 'AND' || key === 'and') {
    const parts = (Array.isArray(value) ? value : [value]).map((v: any) => buildWhere(table, v));
    return parts.length ? and(...parts) : undefined;
  }
  if (key === 'OR' || key === 'or') {
    const parts = (Array.isArray(value) ? value : [value]).map((v: any) => buildWhere(table, v));
    return parts.length ? or(...parts) : undefined;
  }
  if (key === 'NOT' || key === 'not') {
    return sql`not (${buildWhere(table, value)})`;
  }
  const col = (table as any)[key];
  if (value && typeof value === 'object' && !Array.isArray(value) && !('queryChunks' in value) && !('sql' in value)) {
    if ('in' in value) return inArray(col, value.in);
    if ('notIn' in value) return sql`${col} not in ${value.notIn}`;
    if ('gt' in value) return sql`${col} > ${value.gt}`;
    if ('gte' in value) return sql`${col} >= ${value.gte}`;
    if ('lt' in value) return sql`${col} < ${value.lt}`;
    if ('lte' in value) return sql`${col} <= ${value.lte}`;
    if ('contains' in value) return sql`${col}::text ilike ${'%' + value.contains + '%'}`;
    if ('equals' in value) return eq(col, value.equals);
    if ('not' in value && value.not === null) return sql`${col} is not null`;
  }
  if (value === null) return sql`${col} is null`;
  return eq(col, value);
}

function buildWhere(table: Record<string, any>, where: any): any {
  if (where === undefined || where === null) return undefined;
  if (where && typeof where === 'object' && ('queryChunks' in where || 'sql' in where)) {
    return where;
  }
  const conditions: any[] = [];
  for (const [key, value] of Object.entries(where)) {
    const cond = buildCondition(table, key, value);
    if (cond !== undefined) conditions.push(cond);
  }
  return conditions.length ? and(...conditions) : undefined;
}

function buildOrderBy(table: Record<string, any>, orderBy: any): any[] {
  if (!orderBy) return [];
  const result: any[] = [];
  for (const [key, dir] of Object.entries(orderBy)) {
    if (typeof dir === 'object' && dir !== null) {
      for (const [subKey, subDir] of Object.entries(dir)) {
        const col = (table as any)[subKey];
        if (col) result.push(subDir === 'desc' ? sql`${col} desc` : sql`${col} asc`);
      }
    } else {
      const col = (table as any)[key];
      if (col) result.push(dir === 'desc' ? sql`${col} desc` : sql`${col} asc`);
    }
  }
  return result;
}

function pickColumns(table: Record<string, any>, columns: any): Record<string, any> | undefined {
  if (!columns) return undefined;
  const selected: Record<string, any> = {};
  for (const [col, keep] of Object.entries(columns)) {
    if (keep && (table as any)[col]) selected[col] = (table as any)[col];
  }
  return Object.keys(selected).length ? selected : undefined;
}

interface FindOpts {
  where?: any;
  columns?: any;
  orderBy?: any;
  limit?: number;
}

async function runFindFirst(table: any, opts: FindOpts): Promise<any> {
  const db = getDb();
  let q: any = db.select(pickColumns(table, opts.columns) || table).from(table);
  const w = buildWhere(table, opts.where);
  if (w) q = q.where(w);
  const obs = buildOrderBy(table, opts.orderBy);
  for (const ob of obs) q = q.orderBy(ob);
  if (opts.limit) q = q.limit(opts.limit);
  const rows = await q;
  return rows[0] || null;
}

async function runFindMany(table: any, opts: FindOpts): Promise<any[]> {
  const db = getDb();
  let q: any = db.select(pickColumns(table, opts.columns) || table).from(table);
  const w = buildWhere(table, opts.where);
  if (w) q = q.where(w);
  const obs = buildOrderBy(table, opts.orderBy);
  for (const ob of obs) q = q.orderBy(ob);
  if (opts.limit) q = q.limit(opts.limit);
  const rows = await q;
  return rows;
}

const query = new Proxy({} as Record<string, { findFirst: (opts: any) => Promise<any>; findMany: (opts: any) => Promise<any[]> }>, {
  get: (_t, tableName: string | symbol) => {
    const table = TABLES[tableName as string];
    if (!table) return undefined;
    return {
      findFirst: (opts: any) => runFindFirst(table, opts),
      findMany: (opts: any) => runFindMany(table, opts),
    };
  }
});

const dbProxy = new Proxy({} as Record<string, any>, {
  get: (_t, prop: string | symbol) => {
    if (prop === 'query') return query;
    if (prop === '$client') return getDb().$client;
    if (typeof prop === 'symbol') return undefined;
    return (getDb() as any)[prop as string];
  },
  has: (_t, prop: string | symbol) => prop === 'query' || (typeof prop === 'string' && prop in getDb()),
});

export default dbProxy;
export { sql };
export type { DrizzleDB };
