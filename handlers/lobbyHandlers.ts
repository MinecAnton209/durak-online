...
export default function registerLobbyHandlers(io: any, socket: any, sharedContext: any) {
    const { games, addPlayerToGame, broadcastPublicLobbies, checkBanStatus } = sharedContext;

    socket.on('createLobby', async (settings: any) => {
    ...
    socket.on('joinLobby', async ({ gameId, inviteCode, playerName }: any) => {
        ...
        try {
            ...
            if (Object.keys(lobbyGame.players).length >= lobbyGame.settings.maxPlayers) {
                console.log(`[JoinLobby] Lobby is full`);
                return socket.emit('error', { i18nKey: 'error_lobby_full' });
            }

            const betAmount = lobbyGame.settings.betAmount || 0;
            if (betAmount > 0) {
            ...
            addPlayerToGame(socket, lobbyGame, actualPlayerName);
            ...
            io.to(lobbyToJoinId).emit('lobbyStateUpdate', {
                players: Object.values(lobbyGame.players).map((p: any) => ({ id: p.id, name: p.name, rating: p.rating, isVerified: p.isVerified })),
                hostId: lobbyGame.hostId,
                maxPlayers: lobbyGame.settings.maxPlayers
            });
    ...
    socket.on('getLobbyState', ({ gameId }: any) => {
        ...
        if (game) {
            ...
            const playersForLobby = Object.values(game.players).map((p: any) => ({
                id: p.id,
                name: p.name,
    ...
    socket.on('leaveLobby', ({ gameId }: any) => {
        ...
        if (game.playerOrder.length > 0) {
            ...
            if (humanIds.length === 0) {
                ...
                io.to(gameId).emit('lobbyStateUpdate', {
                    players: Object.values(game.players).map((p: any) => ({ id: p.id, name: p.name, rating: p.rating, isVerified: p.isVerified })),
                    hostId: game.hostId,
                    maxPlayers: game.settings.maxPlayers
                });
                (broadcastPublicLobbies as any)();
            }
        } else {
    ...
    socket.on('updateLobbySettings', async ({ gameId, settings }: any) => {
        const game: any = games[gameId];
    ...
    socket.on('kickPlayer', ({ gameId, playerIdToKick }: any) => {
        const game: any = games[gameId];
        if (!game || game.status !== 'waiting' || game.hostId !== socket.id) return;
        ...
        if (game.players[playerIdToKick]) {
            ...
            game.playerOrder = game.playerOrder.filter((id: any) => id !== playerIdToKick);
    ...
    socket.on('addBot', ({ gameId, difficulty }: any) => {
        ...
        game.players[botId] = {
            id: botId,
            name: (botNames as any)[difficulty] || "Bot",
            isBot: true,
