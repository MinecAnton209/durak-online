...
export default function registerGameHandlers(io: any, socket: any, sharedContext: any) {
    const {
        games,
        gameService,
        achievementService,
    ...
    socket.on('forceStartGame', ({ gameId }: any) => {
        const game = (games as Record<string, any>)[gameId];
    ...
    socket.on('startGame', async ({ gameId }: any) => {
    ...
    socket.on('sendMessage', ({ gameId, message }: any) => {
        const game = (games as Record<string, any>)[gameId];
        ...
        if (player.is_muted) {
            ...
            if (sessionUser && sessionUser.mute_until && new Date(sessionUser.mute_until) < new Date()) {
                ...
                db.update(user).set({ is_muted: false, mute_until: null }).where(eq(user.id, player.dbId || sessionUser?.id)).catch((err: any) => console.error(err));
            } else {
    ...
    socket.on('makeMove', ({ gameId, card }: any) => {
        const game = (games as Record<string, any>)[gameId];
        ...
        if (!player.cards.some((c: any) => c.rank === card.rank && c.suit === card.suit)) {
        ...
        if (isDefender) {
            ...
            const isSameRank = game.table.length > 0 && game.table.every((c: any) => c.rank === card.rank);
            ...
            if (isPerevodnoy && isSameRank) {
                ...
                if (nextPlayer && nextPlayer.cards.length >= (game.table.length + 1)) {
                    ...
                    player.cards = player.cards.filter((c: any) => !(c.rank === card.rank && c.suit === card.suit));
            ...
            if (game.table.length > 0 && !game.table.some((c: any) => c.rank === card.rank)) {
    ...
    socket.on('takeCards', ({ gameId }: any) => {
        const game = (games as Record<string, any>)[gameId];
        ...
        if (defender) {
            ...
            if (defender.dbId) {
                db.update(gameParticipant).set({ cards_taken_total: sql`${gameParticipant.cards_taken_total} + ${game.table.length}` }).where(and(eq(gameParticipant.game_id, gameId), eq(gameParticipant.user_id, defender.dbId))).catch((err: any) => console.error(`[Game] Error updating cards_taken_total:`, err.message));
            }
    ...
    socket.on('requestRematch', ({ gameId }: any) => {
        const game: any = (games as Record<string, any>)[gameId];
        if (!game || !game.players[socket.id]) return;
        game.rematchVotes.add(socket.id);
        const remainingPlayers = game.playerOrder.filter((id: any) => game.players[id]);
    ...
    socket.on('reconnectAttempt', async (payload: any) => {
        const requestedGameId = payload && payload.gameId;
        const sessionUser = socket.request.session?.user;
        const deviceId = socket.deviceId;

        if (!requestedGameId || !(games as Record<string, any>)[requestedGameId]) return socket.emit('reconnectFailed');
        ...
        for (const [socketId, player] of Object.entries(game.players)) {
            ...
            if (p.disconnected) {
                if (sessionUser && p.dbId === sessionUser.id) {
                    oldPlayerSocketId = socketId; oldPlayerData = p; break;
                }
                if (!sessionUser && p.isGuest && p.deviceId === deviceId) {
                    oldPlayerSocketId = socketId; oldPlayerData = p; break;
                }
            }
        }
        ...
        if (oldPlayerSocketId && oldPlayerData) {
            const oldPlayerName = (oldPlayerData as any).name;
            clearTimeout((oldPlayerData as any).reconnectTimeout);

            const newPlayerId = socket.id;
            game.players[newPlayerId] = oldPlayerData as any;
            ...
            io.to(gameId).emit('playerReconnected', { playerId: newPlayerId, oldPlayerId: oldPlayerSocketId, name: oldPlayerName });
            gameService.logEvent(game, null, { i18nKey: 'log_player_reconnected', options: { name: oldPlayerName } });
            gameService.broadcastGameState(gameId);
            return;
        }
    ...
    socket.on('requestGameState', ({ gameId }: any) => {
        const g: any = (games as Record<string, any>)[gameId];
        if (g && g.players[socket.id]) gameService.broadcastGameState(gameId);
    });

    socket.on('adminSpectateGame', ({ gameId }: any) => {
        const sessionUser = socket.request.session.user;
    ...
    socket.on('hostChangeTrack', ({ gameId, trackId, trackTitle }: any) => {
        const game: any = (games as Record<string, any>)[gameId];
    ...
    socket.on('hostTogglePlayback', ({ gameId, isPlaying, currentTime }: any) => {
        const game: any = (games as Record<string, any>)[gameId];
        if (!game || socket.id !== game.hostId) return;
    ...
    socket.on('suggestTrack', ({ gameId, trackId, trackTitle }: any) => {
        const game: any = (games as Record<string, any>)[gameId];
