    ...
    if (playerDbIds.length > 0) {
      try {
        await db
          ...
          .where(inArray(user.id, playerDbIds));
    ...
    await db.transaction(async (tx) => {
      ...
      for (const player of allPlayersInGame) {
        if (player && !player.isGuest && player.dbId) {
          ...
          await achievementService.checkPostGameAchievements(game, player, foundUser, newWinStreak, tx);
        ...
        if (txWinners.length > 0) {
          ...
          if (allRegisteredPlayers.length > 0) {
            await tx
              .update(user)
              .set({ coins: sql`${user.coins} + ${betAmount}` })
              .where(inArray(user.id, allRegisteredPlayers.map((p) => p.dbId)));
  ...
  if (game.status === 'waiting') {
    ...
    if (game.playerOrder.length > 0) {
      ...
      if (humanIds.length === 0) {
        ...
        db.update(game).set({ status: 'cancelled' }).where(eq(game.id, game.id)).catch(() => {});
