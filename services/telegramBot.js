...
import { user, achievement, userAchievement, activeSession, game, inboxMessage } from '../db/schema.ts';
...
${t(lang, 'status.version', { version: stats.app.version })}
  ...
  async function showAchievements(ctx, page = 1, isEdit = false) {
    ...
    try {
      ...
      if (unlocked.length === 0) {
        ...
        listText = '\n\n' + pageItems.map((ua) => {
          const ach = allAchievements.find((a) => a.code === ua.achievement_code);
          if (!ach) return '';
  ...
  async function showSessions(ctx, isEdit = false) {
    ...
    try {
      ...
      const otherSessions = sessions.filter((s) => !s.id.startsWith('tg_')); // Simple heuristic
