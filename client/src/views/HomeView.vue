...
<template>
  <div class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-background">
    ...
    <div class="absolute top-4 right-4 flex gap-2 z-10">
      <button @click="setLang('uk')" :class="{ 'grayscale-0 scale-110 drop-shadow-lg': locale === 'uk' }" class="text-2xl hover:scale-110 transition-transform cursor-pointer grayscale-[0.5] hover:grayscale-0" :title="$t('lang_uk')">🇺🇦</button>
      <button @click="setLang('en')" :class="{ 'grayscale-0 scale-110 drop-shadow-lg': locale === 'en' }" class="text-2xl hover:scale-110 transition-transform cursor-pointer grayscale-[0.5] hover:grayscale-0" :title="$t('lang_en')">🇬🇧</button>
      <button @click="setLang('ru')" :class="{ 'grayscale-0 scale-110 drop-shadow-lg': locale === 'ru' }" class="text-2xl hover:scale-110 transition-transform cursor-pointer grayscale-[0.5] hover:grayscale-0" :title="$t('lang_ru')">🇷🇺</button>
    </div>
    ...
    <div
      ...
      <div class="p-6 md:p-8 flex flex-col gap-5">
        ...
        <div v-if="authStore.isAuthChecking" class="flex justify-center py-3">
          <svg class="animate-spin h-8 w-8 text-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
        </div>
        ...
        <div v-else class="flex gap-2 animate-fade-in">
          <div class="flex-1 bg-primary/10 rounded-xl p-2.5 border border-primary/30 flex items-center justify-center gap-2 relative min-w-0">
            <div class="flex items-center justify-center gap-1.5 w-full">
              <span class="text-xl shrink-0">👤</span>
              <span class="text-primary font-bold truncate text-sm md:text-base">{{ authStore.user?.username }}</span>
              <svg v-if="authStore.user?.isVerified" class="w-4 h-4 text-blue-400 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <div v-if="authStore.user?.streak > 3" class="flex items-center text-orange-500 text-xs font-bold bg-orange-500/10 px-1.5 py-0.5 rounded-full border border-orange-500/20 shrink-0">
                <span>🔥</span>
                <span>{{ authStore.user.streak }}</span>
              </div>
            </div>
          </div>
          <button @click="handleLogout" class="px-4 bg-transparent border border-error/30 text-error hover:bg-error/10 hover:border-error rounded-xl transition-colors active:scale-95 shrink-0" :title="$t('logout_title')">
        ...
        <div class="relative flex py-1 items-center">
          ...
          <span class="flex-shrink-0 mx-4 text-outline text-xs uppercase tracking-widest font-semibold">{{ $t('or_separator') }}</span>
          <div class="flex-grow border-t border-outline/30"></div>
        </div>

        <div v-if="!authStore.isAuthenticated" class="space-y-1.5">
          <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider ml-1">{{ $t('guest_name_label') }}</label>
          <div class="relative">
            ...
            <svg class="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-outline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
          </div>
        </div>
