...
<template>
  <div class="h-[100dvh] w-full flex flex-col relative overflow-hidden font-sans bg-background select-none touch-manipulation">
    ...
    <div v-if="!isGameStarted" class="flex-1 flex items-center justify-center p-4 overflow-y-auto">
      <div class="w-full max-w-xl bg-surface/95 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/5 p-6 md:p-8 animate-fade-in text-on-surface my-auto">
        <h2 class="text-center text-on-surface-variant mb-1 text-sm uppercase">{{ $t('room_label') }}</h2>
        <h1 class="text-4xl md:text-5xl text-center font-bold text-primary tracking-widest font-mono uppercase mb-6">{{ urlGameId }}</h1>
        ...
        <div v-else-if="isLoading" class="flex justify-center py-10"><ShufflingLoader /></div>
        ...
        <div v-else class="flex flex-col gap-4">
          ...
          <div class="flex gap-2">
            <input type="text" :value="currentUrl" readonly class="w-full bg-black/20 border border-outline/50 rounded-xl px-4 py-2 text-white text-xs md:text-sm truncate">
            <button @click="copyLink" class="bg-surface border border-outline/50 px-3 rounded-xl hover:bg-white/10 text-white transition-colors">📋</button>
          </div>

          <div v-if="authStore.isAuthenticated" class="flex justify-end">
            <button @click="openFriendsList" class="text-xs font-bold text-primary flex items-center gap-1 bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 hover:bg-white/10 transition-colors">
          ...
          <button v-if="gameStore.isHost" @click="startGame" :disabled="gameStore.players.length < 2" class="w-full bg-primary text-on-primary font-bold py-3 rounded-xl shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all">{{ $t('start_game_button') }}</button>
          <p v-else class="text-center text-xs text-white/50 animate-pulse">{{ $t('waiting_for_host') }}</p>
          <button @click="gameStore.leaveGame" class="w-full bg-transparent border border-white/10 text-white/70 hover:bg-white/5 font-bold py-2 rounded-xl transition-all">{{ $t('leave_lobby') }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-else class="flex flex-col h-full w-full relative">
      ...
      <div class="grid grid-rows-[auto_1fr_auto] h-full w-full transition-opacity duration-700 pb-safe" :class="gameStore.isDealing ? 'opacity-0 pointer-events-none' : 'opacity-100'">
        ...
        <div class="w-full px-2 pt-2 pb-1 shrink-0 flex items-start" :class="opponentsGridClass">
          <div v-for="(opp, oppIdx) in opponents" :key="opp.id" class="flex flex-col items-center relative group transition-all min-w-[4rem] md:min-w-[6rem]">
            <div class="w-10 h-10 md:w-16 md:h-16 rounded-full flex items-center justify-center text-white font-bold text-sm md:text-xl border-2 md:border-4 transition-all z-10 bg-surface shadow-md relative" :class="{'border-primary shadow-[0_0_15px_rgba(0,191,165,0.6)] scale-110': opp.id === gameStore.turnPlayerId, 'border-white/10': opp.id !== gameStore.turnPlayerId, 'opacity-50 grayscale': gameStore.disconnectedPlayers[opp.id]}">
              {{ opp.name[0] }}
              <div v-if="gameStore.disconnectedPlayers[opp.id]" class="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center text-2xl animate-pulse">⏳</div>
              <div v-if="gameStore.attackerId === opp.id" class="absolute -bottom-1 -right-1 text-base md:text-2xl drop-shadow-md animate-bounce z-20">⚔️</div>
              <div v-if="gameStore.defenderId === opp.id" class="absolute -bottom-1 -right-1 text-base md:text-2xl drop-shadow-md animate-pulse z-20">🛡️</div>
            </div>
            <div class="mt-1 bg-black/40 px-1.5 py-0.5 rounded-full backdrop-blur-sm w-full flex justify-center max-w-full z-30">
              <p class="text-white text-[10px] md:text-xs font-medium truncate text-center leading-tight flex-1">{{ opp.name }}</p>
              <span v-if="gameStore.disconnectedPlayers[opp.id]" class="text-orange-400 font-bold text-xs ml-1">{{ gameStore.disconnectedPlayers[opp.id] }}</span>
              <svg v-if="opp.isVerified" class="w-3 h-3 text-blue-400 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <span v-if="opp.streak > 3" class="text-[10px] text-orange-500 font-bold shrink-0">🔥{{ opp.streak }}</span>
            </div>
            <transition-group tag="div" name="opp-cards" class="flex justify-center items-end relative w-full h-16 md:h-20 -mt-2">
              <div v-for="(n, idx) in (opp.cardCount || opp.cards?.length || 0)" :key="idx" class="absolute bottom-0 origin-bottom transition-transform" :style="getOpponentCardStyle(idx, (opp.cardCount || opp.cards?.length || 0))">
                <Card :is-back="true" :card-style="opp.cardBackStyle || 'default'" class="w-8 h-12 md:w-12 md:h-16 shadow-md !rounded-lg border border-black/20 z-20" />
              </div>
            </transition-group>
          </div>
        </div>
        ...
        <div class="relative w-full flex items-center justify-center min-h-0">
          <div class="absolute left-2 top-1/2 -translate-y-1/2 flex items-center z-0 scale-[0.6] md:scale-100 origin-left">
            <Card v-if="gameStore.trumpCard" :rank="gameStore.trumpCard.rank" :suit="gameStore.trumpCard.suit" class="rotate-90 translate-x-12 opacity-100 z-0" />
            <div v-if="gameStore.deckCount > 0" class="relative z-10">
              <Card :is-back="true" class="shadow-xl" card-style="default" />
              <div class="absolute -bottom-6 left-1/2 -translate-x-1/2 text-white font-bold bg-black/60 px-2 rounded text-xs border border-white/10">{{ gameStore.deckCount }}</div>
            </div>
            <div v-else class="absolute left-0 text-white/30 font-bold border-2 border-dashed border-white/10 w-20 h-28 rounded-xl flex items-center justify-center z-0">0</div>
          </div>
          <div class="flex flex-wrap justify-center items-center content-center gap-2 md:gap-4 px-8 md:px-32 w-full h-full max-w-4xl gap-y-6 overflow-auto">
            <div v-for="(card, i) in gameStore.tableCards" :key="`${card.rank}-${card.suit}-${i}`" class="relative transition-all duration-300 transform scale-[0.85] sm:scale-100" :class="{'z-0 ml-1 md:ml-2': i % 2 === 0, 'z-10 -ml-10 md:-ml-16 -mt-4 md:-mt-8 rotate-6 shadow-xl': i % 2 !== 0}">
        ...
        <div class="shrink-0 flex flex-col items-center w-full pb-2 md:pb-4 gap-1 relative z-30">
          <div class="mb-1">
            <div v-if="!gameStore.canPass && !gameStore.canTake" class="px-3 py-1 bg-black/60 backdrop-blur text-white/90 rounded-full text-xs md:text-sm border border-white/10 animate-pulse">
              ...
              <span v-else class="text-primary font-bold">{{ gameStore.isAttacker ? $t('status_your_turn') : $t('status_defend') }}</span>
            </div>
          </div>
          <div class="flex gap-4 w-full px-4 justify-center pointer-events-none h-0 relative z-20">
            <div class="absolute bottom-4 flex gap-4 pointer-events-auto">
              <transition name="pop"><button v-if="gameStore.canPass" @click="gameStore.passTurn" class="px-6 py-2 bg-slate-600 text-white rounded-xl font-bold shadow-[0_0_15px_rgba(0,0,0,0.5)] active:scale-95 border-2 border-slate-400 text-sm md:text-base">{{ $t('btn_pass') }}</button></transition>
              <transition name="pop"><button v-if="gameStore.canTake" @click="gameStore.takeCards" class="px-6 py-2 bg-red-500 text-white rounded-xl font-bold shadow-[0_0_15px_rgba(0,0,0,0.5)] active:scale-95 border-2 border-red-300 text-sm md:text-base">{{ $t('btn_take') }}</button></transition>
            </div>
          </div>
          <div class="w-full overflow-x-auto overflow-y-visible px-4 pb-2 pt-8 no-scrollbar touch-pan-x">
            <div class="flex justify-center min-w-max mx-auto px-4" :class="gameStore.myCards.length > 5 ? '-space-x-8 sm:-space-x-12' : '-space-x-4 sm:-space-x-8'">
              <transition-group name="hand">
                <div v-for="(card, index) in gameStore.myCards" :key="`${card.rank}-${card.suit}`" class="transition-all duration-300 transform origin-bottom cursor-pointer relative hover:-translate-y-4 hover:!z-50 hover:scale-110" :style="{ zIndex: index }" @click="onCardClick(card)">
                  <Card :rank="card.rank" :suit="card.suit" :is-playable="gameStore.canPlayCard(card)" :class="{ 'brightness-50 grayscale-[0.5]': !gameStore.canPlayCard(card) && gameStore.isMyTurn }" />
                </div>
              </transition-group>
            </div>
          </div>
...
.hand-enter-active, .hand-leave-active { transition: all 0.4s ease; }
.hand-enter-from { opacity: 0; transform: translateY(100px); }
.hand-leave-to { opacity: 0; transform: translateY(-100px); }
.hand-move { transition: transform 0.4s ease; position: relative; }
...
.opp-cards-enter-active, .opp-cards-leave-active { transition: all 0.3s ease; }
.opp-cards-enter-from, .opp-cards-leave-to { opacity: 0; transform: scale(0); }
</style>
