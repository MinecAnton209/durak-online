...
<template>
  <div class="min-h-screen bg-[#0f2a1d] flex flex-col items-center p-2 md:p-4 safe-p overflow-y-auto font-sans">

    <div
      class="w-full max-w-4xl flex justify-between items-center mb-4 md:mb-6 sticky top-[var(--safe-area-top)] bg-[#0f2a1d]/90 backdrop-blur-sm z-20 py-2">
    ...
    <div class="w-full max-w-4xl flex flex-col gap-4 md:gap-6 pb-20">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div
          ...
          <h3 class="text-on-surface-variant text-xs md:text-sm uppercase tracking-widest font-bold mb-0 md:mb-2">
            {{ $t('status_label') }}</h3>
          <div class="flex items-center gap-4 md:block md:text-center">
            <div class="text-xl md:text-3xl font-bold" :class="{
              ...
              {{ store.phase === 'betting' ? $t('roulette_phase_betting') : (store.phase === 'spinning' ?
                $t('roulette_phase_spinning') :
        ...
        <div
          ...
          <h3 class="text-on-surface-variant text-xs md:text-sm uppercase tracking-widest font-bold mb-2 md:mb-3">
            {{ $t('roulette_recent_label') }}</h3>
          <div class="flex md:flex-wrap gap-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0 scrollbar-hide">
            <div v-for="(num, idx) in store.history" :key="idx"
              class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border shrink-0" :class="{
