...
<template>
  <div
    class="min-h-screen flex items-center justify-center bg-background relative overflow-hidden font-sans p-4 safe-p select-none">
    ...
    <div
      class="relative z-10 w-full max-w-lg bg-surface/85 backdrop-blur-md rounded-[28px] border border-white/10 p-8 md:p-12 text-center shadow-2xl">
      ...
      <router-link to="/"
        class="inline-block bg-primary hover:bg-[#00A891] text-on-primary font-bold text-lg py-3 px-8 rounded-xl shadow-lg transition-transform active:scale-95">
        {{ $t('go_home') }}
      </router-link>
    </div>
...
@keyframes spin-card {
  from {
    transform: rotateY(0deg) rotateX(5deg) rotateZ(-3deg);
  }
...
.animate-float-4 {
  animation: float4 15s infinite ease-in-out alternate;
}

.animate-float-5 {
  animation: float4 28s infinite ease-in-out alternate;
}
...
@keyframes float4 {
  ...
  to {
    transform: translate(-15vw, 20vh) rotate(25deg);
  }
}
</style>
