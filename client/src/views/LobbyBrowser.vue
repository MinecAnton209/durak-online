...
<template>
  <div class="h-[100dvh] w-full flex items-center justify-center p-4 safe-p bg-background overflow-hidden">
    ...
    <div class="md:hidden w-full max-w-2xl h-full flex flex-col relative">
      ...
      <button @click="isChatModalOpen = true"
        class="fixed bottom-[calc(1.5rem+var(--safe-area-bottom))] right-[calc(1.5rem+var(--safe-area-right))] bg-primary text-on-primary rounded-full w-16 h-16 flex items-center justify-center shadow-lg text-3xl z-40 active:scale-90 transition-transform md:hidden">
        💬
      </button>
    </div>
    ...
    <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-full"
      ...
      <div v-if="isChatModalOpen"
        class="md:hidden fixed inset-0 bg-background/80 backdrop-blur-md z-50 p-4 pt-[calc(2rem+var(--safe-area-top))] flex flex-col safe-px safe-pb">
