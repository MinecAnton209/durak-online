      ...
      <div class="p-6 overflow-y-auto">
        ...
        <div v-if="activeTab === 'create'" class="flex flex-col gap-5 animate-fade-in">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="text-xs font-bold text-on-surface-variant uppercase ml-1">{{ $t('lobby_type') }}</label>
              <select v-model="lobbyType" class="w-full bg-black/20 border border-outline/50 rounded-xl px-4 py-3 mt-1 text-on-surface focus:outline-none focus:border-primary cursor-pointer appearance-none">
            ...
            <div>
              <label class="text-xs font-bold text-on-surface-variant uppercase ml-1">{{ $t('players_count_label') }}</label>
              <select v-model="maxPlayers" class="w-full bg-black/20 border border-outline/50 rounded-xl px-4 py-3 mt-1 text-on-surface focus:outline-none focus:border-primary cursor-pointer appearance-none">
            ...
            <div>
              <label class="text-xs font-bold text-on-surface-variant uppercase ml-1">{{ $t('deck_size_label') }}</label>
              <select v-model="deckSize" class="w-full bg-black/20 border border-outline/50 rounded-xl px-4 py-3 mt-1 text-on-surface focus:outline-none focus:border-primary cursor-pointer appearance-none">
          ...
          <div v-if="isBetting" class="space-y-1.5 animate-fade-in">
            <label class="text-xs font-bold text-primary uppercase ml-1">{{ $t('bet_amount_label') }}</label>
            <div class="relative">
              <input type="number" v-model="betAmount" class="w-full bg-black/20 border rounded-xl px-4 py-3 text-on-surface focus:outline-none border-primary focus:ring-1 focus:ring-primary pl-10 font-bold" min="10" step="10">
              <span class="absolute left-3 top-1/2 -translate-y-1/2 text-primary">💰</span>
            </div>
          </div>

          <button @click="createLobby" class="w-full bg-primary hover:bg-[#00A891] text-on-primary font-bold text-lg py-4 rounded-2xl shadow-lg mt-2 transition-all active:scale-95">
