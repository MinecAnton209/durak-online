...
const fetchLeaders = async () => {
  ...
  try {
    const res = await fetch(getApiUrl(`/api/public/leaderboard?type=${activeTab.value}&limit=50`));
    if (res.ok) {
...
<template>
  <transition name="fade">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 safe-p">
      ...
      <div
        class="relative w-full max-w-lg bg-surface rounded-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[80vh] animate-scale-in">
            ...
            <tbody class="text-sm">
              <tr v-for="(player, idx) in leaders" :key="idx" class="border-b border-white/5 hover:bg-white/5">
                ...
                <td class="p-3 text-right font-mono text-on-surface">
                  {{ activeTab === 'rating' ? Math.round(player.rating) : (activeTab === 'wins' ? player.wins :
                    player.win_streak) }}
                </td>
              </tr>
            </tbody>
