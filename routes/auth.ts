...
async function checkDeviceBan(deviceId: string | undefined): Promise<string | null> {
...
async function recordDeviceActivity(userId: number | undefined, deviceId: string, req: Request, isLogin = false): Promise<void> {
    if (!deviceId) return;
...
router.post('/login', loginLimiter, async (req: Request, res: Response) => {
    try {
        ...
        await db.insert(activeSession).values({ id: sessionId, user_id: userRecord.id, device_info: ua as string, ip_address: ip as string, location });
        ...
        req.session = { user: payload, save() { }, destroy() { } } as any;
        ...
        await inboxService.addMessage(userRecord.id, {
            ...
            contentParams: { ip, location, userAgent: ua, sessionId, deviceId: deviceId ?? null } as any
        });
...
router.get('/api/auth/sessions', async (req: Request, res: Response) => {
...
router.delete('/api/auth/sessions/:id', async (req: Request, res: Response) => {
    const currentUser = req.session?.user as any;
    if (!currentUser) return res.status(401).json({ message: 'Unauthorized' });
    ...
    try {
        ...
        await db.delete(activeSession).where(eq(activeSession.id, targetId as string));
...
router.get('/check-session', async (req: Request, res: Response) => {
    const currentUser = req.session?.user as any || null;
    ...
    if (currentUser && currentUser.id) {
        ...
        try {
            ...
            if (lastPlayed) {
                ...
                const diffDays = Math.ceil((todayMidnight.getTime() - lastMidnight.getTime()) / (1000 * 60 * 60 * 24));
                if (diffDays > 1 && userRecord.streak_count > 0) {
                    currentStreak = 0;
                    db.update(user).set({ streak_count: 0 }).where(eq(user.id, userRecord.id)).catch((e: any) => console.error(e));
                }
            }
            ...
            if (isMuted && muteUntil && new Date(muteUntil) < new Date()) {
                ...
                db.update(user).set({ is_muted: false, mute_until: null }).where(eq(user.id, userRecord.id)).catch((e: any) => console.error(e));
            }
            ...
            if (isBanned && banUntil && new Date(banUntil) < new Date()) {
                ...
                db.update(user).set({ is_banned: false, ban_until: null, ban_reason: null }).where(eq(user.id, userRecord.id)).catch((e: any) => console.error(e));
            }
            ...
            const sessionUser: any = {
                ...
                sessionId: currentUser.sessionId || crypto.randomUUID()
            };
            req.session = { user: sessionUser, save() { }, destroy() { } } as any;
...
router.post('/update-settings', async (req: Request, res: Response) => {
    ...
    const { card_back_style, pref_quick_deck_size, pref_quick_max_players, pref_quick_game_mode, pref_quick_is_betting, pref_quick_bet_amount } = req.body as { card_back_style?: string; pref_quick_deck_size?: string; pref_quick_max_players?: string; pref_quick_game_mode?: string; pref_quick_is_betting?: boolean; pref_quick_bet_amount?: string };
    ...
    try {
        ...
        if (req.session && req.session.user) {
            if (card_back_style) (req.session.user as any).card_back_style = card_back_style;
            if (pref_quick_deck_size !== undefined) (req.session.user as any).pref_quick_deck_size = parseInt(pref_quick_deck_size);
            if (pref_quick_max_players !== undefined) (req.session.user as any).pref_quick_max_players = parseInt(pref_quick_max_players);
            if (pref_quick_game_mode !== undefined) (req.session.user as any).pref_quick_game_mode = pref_quick_game_mode;
            if (pref_quick_is_betting !== undefined) (req.session.user as any).pref_quick_is_betting = pref_quick_is_betting;
            if (pref_quick_bet_amount !== undefined) (req.session.user as any).pref_quick_bet_amount = parseInt(pref_quick_bet_amount);
        }
...
router.post('/change-password', passwordChangeLimiter, async (req: Request, res: Response) => {
    const currentUser = req.session?.user as any;
    if (!currentUser) return res.status(401).json({ message: 'Unauthorized' });
