...
import { and, eq, inArray, isNotNull, gte, sql } from 'drizzle-orm';
    ...
    try {
        ...
        if (gameIds.length > 0) {
            const participants = await db.select({
                ...
                .where(inArray(gameParticipant.game_id, gameIds as string[]));
            for (const p of participants) {
                const gid = p.game_id as string;
                if (!participantsByGame[gid]) participantsByGame[gid] = [];
                participantsByGame[gid].push(p);
            }
        }
