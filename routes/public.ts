...
router.get('/health', async (req: Request, res: Response) => {
    try {
        ...
        const activeGamesMap = (req.app.get('activeGames') as Record<string, any>) || {};
        ...
        for (const game of Object.values(activeGamesMap) as any[]) {
            if (game.status === 'in_progress') {
                gamesInProgress++;
                playersInMatches += game.playerOrder.length;
                if (Object.values(game.players).some((p: any) => p.isBot)) botGames++;
        ...
        if (!dailyStats) dailyStats = { new_registrations: 0, games_played: 0 } as any;
...
router.get('/leaderboard', async (req: Request, res: Response) => {
    const { type = 'rating', limit = 20 } = req.query as { type?: string; limit?: number | string };

    const allowedSortTypes = { rating: 'rating', wins: 'wins', win_streak: 'win_streak' };
    const orderByColumn = (allowedSortTypes as Record<string, string>)[type as string];
    if (!orderByColumn) return res.status(400).json({ error: 'Invalid leaderboard type' });

    const safeLimit = Math.min(Math.max(1, parseInt(limit as string, 10)), 100);
...
router.get('/game/:id/status', async (req: Request, res: Response) => {
    ...
    const gameId = (req.params.id as string).toUpperCase();
