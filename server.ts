...
db.update(gameTable).set({ status: 'cancelled' }).where(eq(gameTable.status, 'waiting'))
    ...
    .catch((err: any) => console.error('DB Clean error:', err));
...
async function checkBanStatus(userId: any) {
    try {
...
io.on('connection', (socket) => {
        ...
        db.query.user.findFirst({
            ...
            if (dbUser && dbUser.is_banned) {
                if (dbUser.ban_until && new Date(dbUser.ban_until) < new Date()) {
                    ...
                    const options: any = { reason: reasonText };
                    if (dbUser.ban_until) {
    ...
    socket.on('leaveLobby', ({ gameId }) => {
        ...
        if (game.playerOrder.length > 0) {
            const humanIds = game.playerOrder.filter((id: string) => game.players[id] && !game.players[id].isBot);
            if (humanIds.length === 0) {
                ...
                io.to(gameId).emit('lobbyStateUpdate', {
                    players: Object.values(game.players as any).map((p: any) => ({ id: p.id, name: p.name, rating: p.rating, isVerified: p.isVerified })),
                    hostId: game.hostId,
    ...
    socket.on('updateLobbySettings', async ({ gameId, settings }) => {
        ...
        try {
            ...
            io.to(gameId).emit('lobbyStateUpdate', {
                players: Object.values(game.players as any).map((p: any) => ({
    ...
    socket.on('kickPlayer', ({ gameId, playerIdToKick }) => {
        ...
        if (game.players[playerIdToKick]) {
            ...
            game.playerOrder = game.playerOrder.filter((id: string) => id !== playerIdToKick);
    ...
    socket.on('reconnectAttempt', async (payload) => {
        ...
        let oldPlayerSocketId: string | null = null;
        let oldPlayerData: any = null;

        for (const [socketId, player] of Object.entries(game.players)) {
            if ((player as any).disconnected) {
                if (sessionUser && (player as any).dbId === sessionUser.id) {
                    oldPlayerSocketId = socketId;
                    oldPlayerData = player;
                    break;
                }
                if (!sessionUser && (player as any).isGuest && (player as any).deviceId === deviceId) {
                    oldPlayerSocketId = socketId;
                    oldPlayerData = player;
                    break;
                }
        ...
        if (!oldPlayerSocketId && sessionUser) {
            for (const [socketId, player] of Object.entries(game.players)) {
                if ((player as any).dbId === sessionUser.id) {
    ...
    socket.on('addBot', ({ gameId, difficulty }) => {
        ...
        game.players[botId] = {
            id: botId,
            name: (botNames as Record<string, string>)[difficulty] || "Bot",
            isBot: true,
